...
class StartWindow(QMainWindow):
    ...
    def update_data(self, data):
        ...
        print('update view',np.mean(data),time.time())
        print(data.shape)
        for i,im in enumerate(self.image_views):
            im.setImage(np.rot90(np.flipud(data[:,:,i])))
            
        
    def update_movie(self):
        frame = self.camera.last_frame
        print('update view',frame.shape,frame.ndim,np.mean(frame))
        if np.sum(frame.shape) > 2 and frame.ndim ==3 :
            #frame = np.transpose(frame,axes=(1,0,2))
            for i,im in enumerate(self.image_views):
                im.setImage(np.rot90(np.flipud((frame[:,:,i]))))

    def start_movie(self):
        self.movie_thread = MovieThread(self.camera)
        self.movie_thread.dataChanged.connect(self.update_data)
